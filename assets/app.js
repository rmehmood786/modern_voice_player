...
function fileNameFrom(url){ try{const u=new URL(url); return decodeURIComponent(u.pathname.split('/').pop());}catch{return url.split('/').pop();}}
const download = (filename, text) => { const a=document.createElement('a'); a.download=filename; a.href=URL.createObjectURL(new Blob([text],{type:'application/json'})); a.click(); setTimeout(()=>URL.revokeObjectURL(a.href),1500); };

/* ===== State ===== */
const SEEK_STEP=30, MIC_THRESHOLD=0.025, QUIET_DELAY=1000, DUCK_TICK=60, VIZ_BARS=72;
let state={
  playlist:[], index:0, isPlaying:false,
  mode:'yt', repeat:false, shuffle:true, duration:0,
...
const els={
  ...
  importList:$('#importList'), exportList:$('#exportList'), importInput:$('#importInput'),
  saveList:$('#saveList'), clearList:$('#clearList'),
  marks:$('#marks'), marksList:$('#marksList'),
};
function setStatus(msg,kind='info'){ if(!els.status) return; els.status.textContent=msg; els.status.style.color=(kind==='error')?'var(--danger)':'var(--muted)'; }
...
function setMeta({title,sub,cover}){ els.trackTitle.textContent=title||'—'; els.trackSub.textContent=sub||'—'; els.miniTitle.textContent=title||'—'; els.miniSub.textContent=sub||'—'; if(cover) els.audioCover.src=cover; }

/* ===== WebAudio graph for local media ===== */
...
function attachMedia(el){
  ...
  }catch(e){ /* already attached once; ignore */ }
}
function setEQ(mode){
...
(function(){
  ...
  (function loop(){
    if(state.mode==='yt'){ els.viz.classList.add('hidden'); }
      ...
      els.viz.classList.remove('hidden');
...
/* ===== Playlist / Storage / Import-Export ===== */
const DEFAULT=[ // used only if both localStorage and file are empty
...
const LS_KEY='vibeboxPlaylist';

function loadStore(){
  // 1) Try localStorage
  let fromLS=null;
  try{ const raw=localStorage.getItem(LS_KEY); fromLS = raw? JSON.parse(raw): null; }catch{}
  if(fromLS && fromLS.length){ state.playlist = fromLS; return Promise.resolve('ls'); }

  // 2) Fallback to assets/playlist.json
  return fetch('assets/playlist.json', {cache:'no-store'})
    .then(r=> r.ok ? r.json() : [])
    .then(arr=>{ state.playlist = Array.isArray(arr) && arr.length ? arr : DEFAULT.slice(); })
    .catch(()=>{ state.playlist = DEFAULT.slice(); });
}
function saveStore(){ localStorage.setItem(LS_KEY, JSON.stringify(state.playlist)); }

function exportPlaylist(){
  download('playlist.json', JSON.stringify(state.playlist, null, 2));
  toast('Exported playlist.json');
}
function importPlaylistFromText(text){
  try{
    // JSON?
    let imported = JSON.parse(text);
    if(!Array.isArray(imported)) throw 0;
    // normalize
    imported = imported.map(x=>({type:x.type||'yt', url:x.url, title:x.title||x.url}));
    state.playlist = imported;
    saveStore(); renderList(); if(state.playlist.length) { state.index=0; loadCurrent(false); }
    toast('Imported JSON playlist');
    return;
  }catch{}
  // TXT: "title | url" per line
  const lines = text.split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
  const items = [];
  for(const line of lines){
    const [titlePart,urlPart] = line.split('|').map(s=>s?.trim());
    if(!urlPart) continue;
    const type = (urlPart.includes('youtube.com') || urlPart.includes('youtu.be')) ? 'yt' : 'local';
    items.push({type, url:urlPart, title:titlePart||urlPart});
  }
  if(items.length){
    state.playlist = items;
    saveStore(); renderList(); if(state.playlist.length){ state.index=0; loadCurrent(false); }
    toast('Imported TXT playlist');
  }else{
    toast('Nothing to import');
  }
}

/* Render list with mobile-friendly actions */
function renderList(){
  els.list.innerHTML='';
  state.playlist.forEach((it,i)=>{
    ...
    sub.textContent = (it.type==='yt') ? 'YouTube' : (it.type==='local-video' ? 'Local Video' : 'Local Audio');
    sub.title = sub.textContent;

    main.appendChild(t); main.appendChild(sub);

    // actions (desktop right, on mobile goes under)
    ...
    const bCopy=document.createElement('button'); bCopy.className='btn ghost'; bCopy.textContent='Copy link';
    bCopy.onclick=()=>{ (navigator.clipboard?.writeText(it.url)||Promise.reject()).then(()=>toast('Copied'),()=>toast('Copy failed')); };
    ...
    act.append(bPlay,bQ,bCopy,bRem);
  ...
  // drag reorder
  let dragEl=null;
  els.list.querySelectorAll('li').forEach(item=>{
...
async function loadCurrent(autoplay=false){
  ...
  // Reset A–B
  state.A=state.B=null; state.loopAB=false; $('#toggleAB') && ($('#toggleAB').textContent='A–B: OFF');
  ...
  if(it.type==='yt'){
    ...
    setMeta({title:it.title || fileNameFrom(it.url), sub:'Local Video'});
    ...
    showStage('audio');
    audioEl.src = it.url; audioEl.muted = true; attachMedia(audioEl);
    try{ await audioEl.play(); }catch{} audioEl.pause();
    audioEl.muted=false; $('#unmuteAUD').classList.add('hidden');
    setMeta({title:it.title || fileNameFrom(it.url), sub:'Local Audio', cover:'https://dummyimage.com/1280x720/0b1020/ffffff&text=Audio'});
...
function crossfade(nextFn){
  if(fading) return nextFn(); fading=true;
  const start=performance.now(), D=350, startVol=state.vol;
  function step(t){
    const k=Math.min(1,(t-start)/D), v=(1-k);
...
function next(auto=false){
  crossfade(()=>{
    ...
    else if(state.repeat && auto){ /* repeat current */ }
...
/* Boot */
async function boot(){
  await loadStore();            // pulls LS or assets/playlist.json
  renderList();
  if(state.playlist.length){ state.index=0; loadCurrent(false); }
